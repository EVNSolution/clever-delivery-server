...
이 문서는 가 무엇을 만들지, 왜 필요한지, 현재 어디까지 확정됐는지를 기록한다. Agent 실행 절차는 루트 를 따른다.
...
- Final server framework: Node/TypeScript Fastify or Express
- Driver app authentication depth for MVP: simple JWT vs managed identity later
- Geocoding/routing provider for MVP

## 다음 작업 목록

1. Scaffold Node/TypeScript server structure.
2. Add Docker Compose for app + PostgreSQL + Nginx-compatible local runtime.
3. Add Prisma schema and initial migrations.
4. Implement Shopify webhook verification and order sync client.
