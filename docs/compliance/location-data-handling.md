...
| `POST /driver/proof-media` | Yes | Not normally location usage unless image metadata/location is stored | Stores proof photo metadata and file bytes under driver/shop/route/stop scope; avoid EXIF/location metadata retention unless explicitly reviewed. |
...
| `DriverProofMedia` rows and stored proof files | 180 days after occurrence/upload by default | Needed for customer support and delivery dispute handling. | Delete stored file bytes and mark/delete metadata after retention unless legal/incident hold exists; strip image location metadata in a hardening pass if uploads can contain EXIF. |
...
- No automatic retention cleanup job yet, including proof-media file deletion.
